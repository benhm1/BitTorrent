	...
	switch (**data) {
		...
		case 'd': {
			...
			while (**data != 'e') {
				...
				if ( 0 == strcmp( ret->val.d[i].key, "info" )) {
				  // We want to save our starting position
				  infoDict = 1;
				  startPtr = (char*) ((*data) - 4);
				}
			...
			if ( infoDict ) {
			  char * endPtr = (char*) *data - 1;
			  int infoLen = (endPtr - startPtr) ;
			  char * infoDict = malloc( infoLen );
			  memcpy( infoDict, startPtr, infoLen );
			  
			  printf("Extracted info dictionary - %d bytes\n", infoLen);

			  // Write out the info-dict portion to a file
			  // that we'll read in later. Keep it simple.
			  FILE * fd = fopen("infoDict.bencoding", "w");
			  if ( ! fd ) {
			    perror("fopen");
			    exit(1);
			  }
			  int remaining = infoLen ;
			  while ( remaining > 0 ) {
			    int written = fwrite( infoDict + infoLen - remaining, 
						  1, remaining, fd );
			    if (written < 0) {
			      perror("fwrite");
			      exit(1);
